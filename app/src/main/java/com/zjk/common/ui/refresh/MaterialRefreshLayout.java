package com.zjk.common.ui.refresh;
...
public class MaterialRefreshLayout extends FrameLayout {
    ...
    private int DEFAULT_PROGRESS_SIZE = 40;

    private int BIG_PROGRESS_SIZE = 60;

    private int PROGRESS_STOKE_WIDTH = 2;
    ...
    private MaterialHeadView.OnHeadViewOnAttachToWindowListener mOnAttachListener;
    ...
    protected void onAttachedToWindow() {
        ...
        materialHeadView.setWaveColor(isShowWave ? waveColor : Color.TRANSPARENT);
        ...
        materialHeadView.setOnAttatchListener(mOnAttachListener);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (dy > mSlop && !canChildScrollUp() && refreshEnable) {
                    if (materialHeadView != null) {
                        materialHeadView.onBegin(this);
                    }
                    return true;
                } else if (dy+mSlop < 0 && !canChildScrollDown() && isLoadMore) {
    ...
    public void autoRefresh() {
        if (!isRefreshing) {
            if (materialHeadView!=null) {
                materialHeadView.onBegin(this);
                materialHeadView.onPull(this, 1);
            }
            updateListener();
            if (mHeadLayout!=null) {
                if (isOverlay) {
    ...
    public boolean canChildScrollUp() {
        ...
        if (Build.VERSION.SDK_INT < 14) {
            if (mChildView instanceof AbsListView) {
                ...
                return ViewCompat.canScrollVertically(mChildView, -1) || mChildView.getScrollY() > 0 || upCondition(mChildView);
            }
        } else {
            return ViewCompat.canScrollVertically(mChildView, -1) || upCondition(mChildView);
        }
    }
    ...
    public boolean canChildScrollDown() {
        ...
        if (Build.VERSION.SDK_INT < 14) {
            if (mChildView instanceof AbsListView) {
                ...
                return ViewCompat.canScrollVertically(mChildView, 1) || mChildView.getScrollY() > 0 || downCondition(mChildView);
            }
        } else {
            return ViewCompat.canScrollVertically(mChildView, 1) || downCondition(mChildView);
        }
    }

    protected boolean upCondition(View childView) {
        return false;
    }

    protected boolean downCondition(View childView) {
        return false;
    }
    ...
    public void setAttachListener (MaterialHeadView.OnHeadViewOnAttachToWindowListener listener) {
