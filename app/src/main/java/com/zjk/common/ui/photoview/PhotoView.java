package com.zjk.common.ui.photoview;
 ...
 * 开源控件 https://github.com/bm-x/PhotoView
 */
...
//                to = mMaxScale;
                ...
                if (mOnImageScaleListener != null) {
                    mOnImageScaleListener.onEnLarge();
                }
            }
    ...
    private float computeScale() {
        ...
        float currH = mImgRect.height();
        float scale = height / currH;
        if (scale < 1f) {
            scale = 1f;
        }
        return scale;
    ...
    private OnImageScaleListener mOnImageScaleListener;

    public void setOnImageScaleListener(OnImageScaleListener listener) {
        mOnImageScaleListener = listener;
    }
