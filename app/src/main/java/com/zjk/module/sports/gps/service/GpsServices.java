...
import com.zjk.module.sports.gps.model.GpsSportsData;
...
//            ActivityCompat.requestPermissions(getBaseContext(), new String[]{Manifest.permission.ACCESS_FINE_LOCATION}, REQUEST_FOR_ACCESS_FINE_LOCATION);
        } else {
    ...
    public void updateNotification(boolean asData) {
        ...
        if (asData) {
            builder.setContentText(String.format(getString(R.string.notification_text),
                    String.valueOf(data.getMaxSpeed()), String.valueOf(data.getDistance())));
        } else {
            builder.setContentText(String.format(getString(R.string.notification_text), '-', '-'));
        }
